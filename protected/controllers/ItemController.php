...
class ItemController extends Controller
{
	public function actionList()
	{
		...
		if(Jii::param('itemid')){
			...
			if(isset($this_item) && !empty($this_item)){
				$meta_title = '';
				$meta_description = '';
				$meta_keywords = '';
				
				if(!empty($this_item->itm_name))$meta_title = strip_tags($this_item->itm_name);
				//if(!empty($this_item->con_metakeywords))$meta_keywords = strip_tags($this_item->con_metakeywords);
				//if(!empty($this_item->con_metadescription))$meta_description = strip_tags($this_item->con_metadescription);
				...
				if(isset($desc) && !empty($desc) && is_array($desc)){
					foreach($desc as $d){
						...
						if(is_array(json_decode($d->save_value))){
							...
							$meta_keywords .= $d->save_value.',';
						}
					}
				}
				
				if(!empty($ads->ads_name))$meta_title = strip_tags($ads->ads_name);
				//if(!empty($ads->con_metakeywords))$meta_keywords = strip_tags($ads->con_metakeywords);
				if(!empty($ads->ads_description))$meta_description = strip_tags($ads->ads_description);
				JMeta::set($meta_title,$meta_keywords,$meta_description);
			}
	...
	public function actionSavelocation(){
		...
		if(Jii::isAjax()){
			...
			$cookie = new CHttpCookie('location', Jii::param('locationid'));
			$cookie->expire = time()+60*60*24*180; 
			Yii::app()->request->cookies['location'] = $cookie;
			//Yii::app()->request->cookies['location'] = new CHttpCookie('location', Jii::param('locationid'));
