<?php
	...
	if(isset(Jii::app()->user->location)){
		$location = Location::model()->findByPk(Jii::app()->user->location);
	}
	if(isset($location)){
		if($location->loc_type == 'City'){
			$city = $location->loc_id;
			$get_country = Location::model()->findByPk($city)->loc_parentid;
		}else{
			$get_country = $location->loc_id;;
		}
	}
	
	if(isset($data['country']) && !empty($data['country'])){
		$country = $data['country'];
	}else if(isset($get_country) && !empty($get_country)){
		$country = $get_country;
	}else{
		$country  = '';
	}
...
<div class="sp-field search-lot sep search-form">
	<div class="input">
		<input type="text" id="keywords" name="keywords" value="<?php echo Jii::param('keywords') ?>" placeholder="Keywords / Member" />
		<span class="ex">eg. BMW, Apartment, Closet...</span>
	</div>
	<div class="input">
	...
	<div class="sp-field">
        <div class="input">
        	<?php echo CHtml::dropDownList('subsubcategory',$data['subsubcategory'],array(),array('prompt'=>'Select Sub Category')); ?>	
        </div>
    </div>
	<div class="sp-field" style="display:none;">
        <div class="input">
        	<?php echo CHtml::dropDownList('subsubsubcategory',$data['subsubsubcategory'],array(),array('prompt'=>'Select Sub Category')); ?>	
        </div>
    </div>
	
	<div class="sp-field">
        <div class="input">
        	<?php echo CHtml::dropDownList('country',$country,$data['countries'],array('prompt'=>'Specify Country')); ?>	
        </div>
    </div>
	<div class="sp-field">
        <div class="input">
...
$(document).ready(function(){
	...
	$('#category').change(function(){
		if($(this).val() != ''){
			ajaxSearch('getSubCategories',{category:$(this).val()},function(data){
				...
				subsubcategory.append('<option value="">Specify Sub Category</option>');
				subsubsubcategory.append('<option value="">Specify Sub Category</option>');
	...
	$('#subcategory').change(function(){
		if($(this).val() != ''){
			...
			ajaxSearch('getSubCategories',{category:$(this).val()},function(data){
				...
				subcategory.append('<option value="">Specify Sub Category</option>');
				subsubcategory.append('<option value="">Specify Sub Category</option>');
				for(var i in data.list){
					subcategory.append('<option value="'+data.list[i]['id']+'">'+data.list[i]['name']+'</option>');
				}
	...
	$('#country').change(function(){
		if($(this).val() != ''){
			ajaxSearch('getCities',{country:$(this).val()},function(data){
				...
				if(city_selected == '') city_selected = <?php echo isset(Jii::app()->user->location)?Jii::app()->user->location:'';?>;
				city.empty();
	...
	$('#search-panel input[type=text]').bind("enterKey",function(e){
	   //do stuff here
	   //alert('i press enter');
	   submitSearchForm();
	});
