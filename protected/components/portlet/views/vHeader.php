...
<div class="country-container">
	...
	<?php
		...
		$criteria->with = array('children'=>array('order'=>'children.loc_position DESC,children.loc_name ASC'));
		$criteria->order = 't.loc_position ASC';
								...
								if(isset($loc->children) && !empty($loc->children) && is_array($loc->children)){
									...
									<ul class="country-box">
										...
										foreach($loc->children as $c){
											...
											<li>
												<a href="javascript://">
													<span id="<?php echo $c->loc_id;?>"><img style="display:none;" width="15" height="10" src="<?php echo Yii::app()->request->baseUrl.''.$c->loc_logo;?>" onError="this.src='<?php echo Jii::app()->baseUrl.'/assets/notfound.jpg'; ?>'"/></span>
													<?php echo $c->loc_name;?>
												</a>
											</li>
											<?php
...
<?php
if(isset(Jii::app()->user->location) && empty(Jii::app()->user->location)){
?>
	<a id="no_locations" class="fancybox" href="javascript://" style="display:none;" title="">Inline</a>
	<div id="inline1-bg" style="display:none;">
		<div id="inline1">
			<h3 style="font-size:18px;">Kindly select your location</h3>
			<ul class="location-box" id="not-active">
				<?php
				foreach($locations as $loc){
					?>
...
$('document').ready(function() {
	var id = <?php echo isset(Jii::app()->user->location)?Jii::app()->user->location:'';?>;
	
	$('#no_locations').click(function(){
		$('#inline1-bg').show();
	});
	//if(id == '') $('#no_locations').trigger('click');
	...
	$('#inline1 .location-box a').click(function(e){
		var elem = $(this);
	...
	$('#inline1 .location-box li').each(function(i, e){
		var that = $(this);
		that.find('.loc-shape').click(function(){
			$('.country-box').slideUp('slow');
			that.find('.country-box').show();
		});
	});
	...
	$('.country-container .location-box li').each(function(i, e){
		var that = $(this);
		that.find('.loc-shape').click(function(){
			$('.country-box').slideUp('slow');
			that.find('.country-box').show();
	...
	<ul>
		...
		<?php }else{?>
			...
			<li style="padding:0 5px; position:relative;" class="sep">
				<span style="font-weight:bold; float:left; width:105px; text-align:right;"><?php echo Jii::t('Welcome').' '.Jii::app()->user->firstname;?></span> <a style="display:inline; padding:0 0 0 3px; float:left;"  href="<?php echo Jii::app()->createUrl('web/logout');?>">,Logout</a>
				<?php
...
<div class="login-box" id="not-active">
	...
	?>
	<div class="clear"></div>
	<div id="forgot_password"><a href="<?php echo Jii::app()->createUrl('web/forgotPassword');?>"><?php echo Jii::t('Forgot Password?');?></a></div>
</div>
<?php }?>
<div class="clear"></div>
