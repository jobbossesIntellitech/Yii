...
if(isset($ads) && !empty($ads)){
	$reference = (!empty($ads->ads_reference))?$ads->ads_reference:'';
	$name = (!empty($ads->ads_name))?$ads->ads_name:'';
	$price = (!empty($ads->ads_price))?$ads->ads_price:'';
	//$location = (!empty($ads->ads_locationid))?$ads->ads_locationid:'';
	$description = (!empty($ads->ads_description))?$ads->ads_description:'';
	
	$location = Location::model()->findByPk($ads->ads_locationid);
	$area = 0;
	$city = 0;
	$country = 0;
...
<div class="list-page">
	<div class="list-subcategories">
		<div class="title-box" style="border:none;">
			<span class="titillium" style="font-size:20px;"><?php echo Jii::t('Edit an Ad'); ?></span> 
		</div>
		<div class="clear"></div>
	</div>
</div>
<div class="form-generator" id="formGenerator_<?php echo $this->id; ?>">
	<form method="post" action="<?php echo Jii::app()->createUrl('web/editads',array('id'=>$this->id,'itemid'=>$this->itemid,'adsid'=>Jii::param('adsid'),'country'=>Jii::param('country'),'city'=>Jii::param('city'))); ?>" enctype="multipart/form-data" >
				...
				<fieldset>
					...
					if(isset($section['fields']) && is_array($section['fields']) && !empty($section['fields'])){
						...
						foreach($section['fields'] AS $g=>$field){
							...
							$criteria->addCondition('save_fieldid = '.$g);
							$criteria->addCondition('save_requestkey = "'.$ads->ads_saverequestkey.'"');
							$save_value = FormSave::model()->find($criteria);
							$selected_value = '';
							if(!empty($save_value)) $selected_value = $save_value->save_value;
							//echo $save_value->save_value.'<br>';
							...
							switch($field['type']){
								...
								break;
								case 'text':
									$f .= $this->template('text',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g,'selected_value'=>$selected_value));
								break;
								case 'password':
									$f .= $this->template('password',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g));
								break;
								case 'select':
									$f .= $this->template('select',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g,'selected_value'=>$selected_value));
								break;
								case 'checkbox':
									$f .= $this->template('checkbox',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g));
								break;
								case 'radio':
									$f .= $this->template('radio',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g,'selected_value'=>$selected_value));
								break;
								case 'date':
								...
								break;
								case 'textarea':
									$f .= $this->template('textarea',array('field'=>$field,'sec_id'=>$k,'fld_id'=>$g,'selected_value'=>$selected_value));
								break;
								case 'email':
					...
					<tr>
						...
						<td><input id="reference" type="text" name="reference" class="validate required"  value="'.$reference.'" /><div class="error-message error">This Field is required!!</div></td>
					</tr>';
					
					echo '<table>
					<tr>
						<th><label>Title</label><span class="required">*</span></th>
						<td><input id="name" type="text" name="name" class="validate required" value="'.$name.'" /><div class="error-message error">This Field is required!!</div></td>
					</tr>
					<tr>
						<th><label>Price</label><span class="required">*</span></th>
						<td><input id="price" type="text" name="price" class="validate required" value="'.$price.'" /><div class="error-message error">This Field is required!!</div></td>
					</tr>
					...
					<tr>
						...
						<td>
							<textarea id="description" name="description" class="validate required">'.$description.'</textarea>
							<div id="fake_description" contenteditable="true">'.$description.'</div>
							<div class="error-message error">This Field is required!!</div>
						</td>
					</tr>
					'.$loc.'
					<tr>
						...
						<td><input id="address_mapping" type="text" name="address_mapping" class="validate" /><div class="error-message error">This Field is required, please select a Area!!</div></td>
					</tr>
					<tr>
						...
						<td style="width:550px;">
							...
							<div style="color:#B10303; margin:5px; padding:5px;">You need to re-enter your Images!!</div>
						</td>
					</tr>
					</table>';
					?>
		...
		?>
		<div class="submit">
			<input type="submit" value="Preview" id="send_<?php echo $this->id; ?>" />
			<div class="clear"></div>
		</div>
	</form>
</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('.form-generator #country').val(selected_country);
		...
		$('.form-generator #area').trigger('change');
...
$(document).ready(function(){
	...
	var this_formid = '<?php echo $ads->ads_itemid;?>';
	var parent_formid = '<?php echo Item::model()->findByPk($ads->ads_itemid)->itm_parentid;?>';
	...
	if(parent_formid == '18'){
		...
		$('.form-generator #field_198_1325').trigger('change');
	}
	...
	if(parent_formid == '29'){
		...
		$('.form-generator #field_199_1336').trigger('change');
	}
