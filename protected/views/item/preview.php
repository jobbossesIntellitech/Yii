		...
		if(isset($subcat) && !empty($subcat) && is_array($subcat)){
			...
			$count = count_items($s->itm_id,0); 
				if(!empty($count)){
					?>
						<div class="item">
							<a class="titillium" href="<?php echo Jii::app()->createUrl('item/list',Item::urlOptions($s->itm_id,$s->itm_name));?>"><?php echo $s->itm_name; ?></a>
						</div>
					<?php
				}
			}
						...
						if(isset($relatedads) && !empty($relatedads) && is_array($relatedads)){
							...
							?>
								<div class="item">
									...
									<div class="desc">
										<div class="name">
											<a href="<?php echo Jii::app()->createUrl('item/preview',Ads::urlOptions($rel->ads_id,$rel->ads_itemid,$rel->ads_name));?>">
												<?php echo $rel->ads_name;?>
											</a>
										</div>
										<div class="clear"></div>
										...
										<div <?php if($rel->ads_price == 0) echo 'style="display:none;"';?> class="price">
											...
											<span class="currency"><?php $cur = Currency::model()->findByPk($rel->ads_currencyid); if(!empty($cur)) echo $cur->cur_sign;?></span>
										</div>
									</div>
									<div class="clear"></div>
								</div>
