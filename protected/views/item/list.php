...
<div class="list-page">
	...
	<div class="list-subcategories">
		...
		if(isset(Jii::app()->user->location)){
			$children_locations = Location::model()->getchildren(Jii::app()->user->location);
			$children_locations[] = 0;
		}
		$disp = 0;
				...
				//if(!empty($count)){
					?>
						<div class="item" <?php echo ($disp >= 20)?'style="display:none;"':'';?>>
							<a class="titillium" href="<?php echo Jii::app()->createUrl('item/list',Item::urlOptions($s->itm_id,$s->itm_name));?>"><?php echo $s->itm_name; ?><span> (<?php $cc = count_items($s->itm_id,0,$children_locations); if(!empty($cc)){echo $cc;}else{echo '0';} ?>)</span></a>
						</div>
					<?php
					$disp++;
				//}
		...
		<div class="list-samelevel">
			...
			if(isset($samelevel) && !empty($samelevel) && is_array($samelevel)){
				...
				//if(!empty($count)){
						?>
							<div class="item" <?php echo ($disp >= 20)?'style="display:none;"':'';?>>
								<a class="titillium" href="<?php echo Jii::app()->createUrl('item/list',Item::urlOptions($s->itm_id,$s->itm_name,true));?>"><?php echo $s->itm_name; ?><span> (<?php $cc = count_items($s->itm_id,0,$children_locations); if(!empty($cc)){echo $cc;}else{echo '0';} ?>)</span></a>
							</div>
						<?php
						$disp++;
					}
...
<?php if(isset($listads) && !empty($listads) && is_array($listads)){ ?>	
	...
	<?php 
	$sortUrl = Item::urlOptions(Jii::param('itemid'),'');
	?>
	<ul class="sorting-box" id="not-active">
		<?php if((Jii::param('itemid') == 10) || ((Item::model()->findByPk(Jii::param('itemid'))->itm_parentid) == 10) || (Jii::param('itemid') == 11) || ((Item::model()->findByPk(Jii::param('itemid'))->itm_parentid) == 11)) {?>
			<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_id','order'=>'ASC'));?>" id="date_old_to_new">Date posted -- oldest to newest</a></li>
			<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_id','order'=>'DESC'));?>" id="date_new_to_old">Date posted -- newest to oldest</a></li>
		<?php }else{ ?>
		<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_price','order'=>'ASC'));?>" id="price_low_to_high">Price -- lowest to highest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_price','order'=>'DESC'));?>" id="price_high_to_low">Price -- highest to lowest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_id','order'=>'ASC'));?>" id="date_old_to_new">Date posted -- oldest to newest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('item/list',$sortUrl+array('sort'=>'ads_id','order'=>'DESC'));?>" id="date_new_to_old">Date posted -- newest to oldest</a></li>
		<?php } ?>
	</ul>
	...
	if(isset($listads) && !empty($listads) && is_array($listads)){
		...
		?>
			<div class="item" data-price="<?php echo isset($l->ads_price)?$l->ads_price:'';?>" data-date="<?php echo isset($l->dates->dat_creation)?strtotime($l->dates->dat_creation):'';?>">
				<div class="image">
					<?php 
					$image = explode(',',$l->ads_gallery);
					
					if(isset($image[0]) && !empty($image[0])){
						//echo Jii::app()->basePath.'/assets/uploads/ads/'.$l->ads_id.'/'.$image[0];
						$img = new ImageProcessor(Jii::app()->params['rootPath'].'/assets/uploads/ads/'.$l->ads_id.'/'.$image[0]);
						$img->setIdentity('list_ads_2');
						$img->resizeIntoZoomIn(170,110);
						echo '<img width="170" height="110" src="'.$img->output(IMAGETYPE_PNG).'"/>';
					}else{
						echo '<img width="170" height="110" src="'.Jii::app()->baseUrl.Jii::notfound().'"/>';
					}
					?>
				...
				<div class="title">
					<a class="name titillium" href="<?php echo Jii::app()->createUrl('item/preview',Ads::urlOptions($l->ads_id,$l->ads_itemid,$l->ads_name));?>">
						<?php echo $l->ads_name; ?>
					</a>
					<div class="clear"></div>
				...
				<div class="member-container">
					...
					<div class="clear"></div>
					<div class="view-profile"><a href="<?php echo Jii::app()->createUrl('web/viewprofile',array('id'=>$l->ads_memberid));?>"><?php echo Jii::t('Shop Owner');?></a></div>
					<div class="clear"></div>
					<div class="member-image">
						<a href="<?php echo Jii::app()->createUrl('web/viewprofile',array('id'=>$l->ads_memberid));?>">
						<?php
					...
					<div class="member-name">
						<div class="listed-text"><a href="<?php echo Jii::app()->createUrl('web/viewprofile',array('id'=>$l->ads_memberid));?>"><?php echo $user->mbr_firstname.' '.$user->mbr_lastname;?></a></div>
					</div>
					<div class="clear"></div>
				</div>
				...
				<div class="location-box">
					<?php echo 'Location: '.Location::breadcrumblocation($l->ads_locationid,true,$l->ads_map);?>
				</div>
				<div class="clear"></div>
...
?>
	<div class="empty-list"><?php echo Jii::t('0 results');?></div>
<?php
}
?>
...
function count_items($itemid,$count,$children_locations){
	...
	if(isset(Jii::app()->user->location)){
		$criteria->addCondition('ads_locationid in ('.implode(',',$children_locations).','.Jii::app()->user->location.')');
	}
...
$('document').ready(function() {	
	...
	$('#number_ads_page #number_of_ads option').parent().change(function(){
