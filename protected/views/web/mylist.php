...
<div class="list-page">
	<div class="list-subcategories">
		<div class="title-box" style="border:none;">
			<span class="titillium" style="font-size:20px;"><?php echo Jii::t('My Ads'); ?></span> 
		</div>
		<div class="clear"></div>
	</div>
	...
	<ul class="sorting-box" id="not-active">
		<li><a href="<?php echo Jii::app()->createUrl('web/myads',array('sort'=>'ads_price','order'=>'ASC'));?>" id="price_low_to_high">Price -- lowest to highest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('web/myads',array('sort'=>'ads_price','order'=>'DESC'));?>" id="price_high_to_low">Price -- highest to lowest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('web/myads',array('sort'=>'ads_id','order'=>'ASC'));?>" id="date_old_to_new">Date posted -- oldest to newest</a></li>
		<li><a href="<?php echo Jii::app()->createUrl('web/myads',array('sort'=>'ads_id','order'=>'DESC'));?>" id="date_new_to_old">Date posted -- newest to oldest</a></li>
	</ul>
	<div class="clear"></div>
	...
	if(isset($listads) && !empty($listads) && is_array($listads)){
		...
		?>
			<div class="item" data-price="<?php echo isset($l->ads_price)?$l->ads_price:'';?>" data-date="<?php echo isset($l->dates->dat_creation)?strtotime($l->dates->dat_creation):'';?>">
				...
				<div class="title">
					<a class="name titillium" href="<?php echo Jii::app()->createUrl('web/adspreview',array('adsid'=>$l->ads_id));?>"><?php echo $l->ads_name; ?></a>
					<div class="clear"></div>
				...
				<div id="status_ads">
					<a class="button right" href="javascript://" id="delete" data-index="<?php echo $l->ads_id;?>">Delete Ad</a>
					<div class="clear"></div>
					<?php
						if($l->ads_status == Ads::status()->getItem('draft')->getValue()){
							echo '<a href="'.Jii::app()->createUrl('web/adspreview',array('adsid'=>$l->ads_id)).'">(Waiting your confirmation)</a>';
						}else if($l->ads_status == Ads::status()->getItem('pending')->getValue()){
...
$('document').ready(function() {	
	...
	$('#number_ads_page #number_of_ads option').parent().change(function(){
		window.location.href = '<?php echo Jii::app()->createUrl('web/myads',array('sort'=>Jii::param('sort'),'order'=>Jii::param('order')));?>&number_ads='+$(this).val();
		//alert($(this).val());
	});
	
	$('#status_ads #delete').click(function(){
		var r = confirm("Are you Sure?");
		if(r == true){
			ajaxDeleteads($(this).attr('data-index'));
		}
	});
	
});

function ajaxDeleteads(id){
	var url = '<?php echo Jii::app()->createAbsoluteUrl('web/deleteads');?>?adsid='+id;
	$.ajax({
		type : 'post',
		url : url,
		data : '',
